...
if (qParams.start == null) {
  var paramsStart = getParameterByName('start');
      qParams.start = (paramsStart) ? paramsStart : 0;
}
...
function checkQuery() {
	...
	if (match == false && document.getElementById('adlBasicSearch').q.value.length > 0){
		...
		return false;
    // remove diacritics
    // commenting this out since we are returning already
    // document.getElementById('adlBasicSearch').q.value= removeDiacritics(document.getElementById('adlBasicSearch').q.value)
	}
}
...
function prevResults() {
  ...
  qParams['start'] = parseInt(qParams['start'], 10) - chunkSize;
  reQuery();
}

function nextResults() {
  window.event.preventDefault();
  qParams.start = parseInt(window.qParams.start, 10) + chunkSize;
	reQuery();
}
...
function reQuery() {
	...
	window.document.location = locbits[0] + qString;
}
